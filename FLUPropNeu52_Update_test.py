...
def show_reinstoffe():
    ...
    def fluid_info(selected_fluid):
        ...
        tp_temp_label["text"] = "- Temperatur: " + str(round(tp_temp,3)) + " Kelin"
    ...
    diagram_combobox = ttk.Combobox(diagram_frame, width=40, textvariable=selected_diagram, values=diagrams, state="readonly")
    ...
    def plot_saturation_lines_general(ax, fluid, diagram_type="T-s-Diagramm"):
        ...
        try:
            ...
            if diagram_type == "T-s-Diagramm":
                ...
                v = 1 / rho
                p = CoolProp.PropsSI("P", "T", T_t, "Q", 0, fluid_name)
                y = T_t if diagram_type == "T-v-Diagramm" else p
                x = v if diagram_type == "T-v-Diagramm" else v
        ...
        try:
            T_c = T_crit
            if diagram_type == "T-s-Diagramm":
                s = CoolProp.PropsSI("S", "T", T_c, "Q", 0, fluid_name)#color=stil_krit["farbe"], marker=stil_krit["marker"], s=10,stil_krit["größe"] * 10
                ...
                v = 1 / rho
                p = CoolProp.PropsSI("P", "T", T_c, "Q", 0, fluid_name)
                y = T_c if diagram_type == "T-v-Diagramm" else p
                x = v if diagram_type == "T-v-Diagramm" else v
    ...
    def generate_isolines_general(fluid_name, ax, x_range, y_range, isoline_type='isobar', num_lines=5, x_axis='S', y_axis='T'):
        ...
        if isoline_type == 'isobar':
            ...
            T_vals = np.linspace(
                CoolProp.PropsSI("Ttriple", fluid_name) + 1,
                CoolProp.PropsSI("Tcrit", fluid_name) - 1,
                100
            )
            s_vals = []
            for T in T_vals:
    ...
    def create_figure(selected):
       ...
       # v = get_column_values("volume")
        ...
        if selected == "T-s-Diagramm":
            ...
            if not minx_var.get() or not maxx_var.get():  # Falls keine benutzerdefinierten Werte gesetzt wurden, max und min werte des Wertebereichs der Tabelle
                minx = 0
                maxx = 9000   
            else:
                minx = float(minx[0])
                maxx = float(maxx[0])    
            if not miny_var.get() or not maxy_var.get():  # Falls keine benutzerdefinierten Werte gesetzt wurden
                miny = 270
                maxy = 700  
            else:
                miny= float(miny[0])
            ...
            if not h or not p:  # Prüft, ob eine der Listen leer ist, bei leerer Liste gibts fehler
                ...
                p = [10000, 50000000]  # Beispielwerte 
            # Berechnung der minimalen und maximalen Werte, falls noch keine Werte angegeben sind
    ...
    checkbox_vars = [checkbox_var, isobar_var, isotherm_var, isochor_var, isentropic_var, isenthalpic_var, isovapore_var]
...
def show_kreisprozesse(): 
    ...
    def plot_saturation_lines_general(ax, fluid, diagram_type="T-s-Diagramm"):
        ...
        try:
            ...
            if diagram_type == "T-s-Diagramm":
                s = CoolProp.PropsSI("S", "T", T_c, "Q", 0, fluid_name)
                ax.scatter(s, T_c,label="Kritischer Punkt", color=stil_krit["farbe"], marker=stil_krit["marker"], s=stil_krit["größe"] * 5, zorder=9)
            elif diagram_type == "log(p)-h-Diagramm":
                h = CoolProp.PropsSI("H", "T", T_c, "Q", 0, fluid_name)
                p = CoolProp.PropsSI("P", "T", T_c, "Q", 0, fluid_name)
                ax.scatter(h, p, label="Kritischer Punkt", color=stil_krit["farbe"], marker=stil_krit["marker"], s=stil_krit["größe"] * 5, zorder=9)
    ...
    checkbox_vars = [isobar_var, isotherm_var, isochor_var, isentropic_var, isenthalpic_var, isovapore_var]
